...
Da cui la tesi.
\end{proof}
\begin{thm}[Converti in Fibonacci]
	Se $\Fib{n}$ è la successione di Fibonacci e  $\Luc{n}$ è quella di Lucas allora:
	\begin{equation}
...
\begin{proof}
	...
	Da cui la tesi.
\end{proof}
Una generalizzazione del~\vref{thm:LucFibSommaprodotto} è il seguente
\begin{thm}[Formule moltiplicaione scalare]\label{thm:LucFibmoltscalare}
	Se $\Fib{n}$ è la successione di Fibonacci e  $\Luc{n}$ è quella di Lucas allora:
	\begin{equation}
...
\end{thm}\cite{Rabinowitz_1996}
 ...
 \begin{align*}
 	...
 	n=&n(k-1)\\
 	m=&n\\
 	\Fib{kn}=&\dfrac{\Fib{n(k-1)}\Luc{m}+\Luc{n(k-1)}\Fib{n}}{2}
 \end{align*}
 \end{proof}