...
\begin{thm}[Formule moltiplicazione scalare]\label{thm:LucFibmoltscalare}
	Se $\Fib{n}$ è la successione di Fibonacci e  $\Luc{n}$ è quella di Lucas allora:
	\begin{equation}
...
\begin{align*}
	...
	=&\Fib{m+n}-\dfrac{a^mb^n+a^nb^m}{(a-b)^2}\\
	\Fib{m-n}=&\dfrac{a^{m-n}}{a-b}+\dfrac{b^{m-n}}{b-a}=a^{-n}b^{-n}(\dfrac{a^mb^n+a^nb^m}{a-b})\\
	\intertext{ma}
	a^mb^n+a^nb^m=&a^{n}b^{n}\Luc{m-n}\\
	\intertext{quindi}
	=&\dfrac{\Luc{m+n}}{(a-b)^2}-\dfrac{a^{n}b^{n}\Luc{m-n}}{(a-b)^2}\\
